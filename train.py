...
if __name__ == '__main__':
    ...
    for epoch in range(epoch_num):
        ...
        for i, data in enumerate(train_data_loader):
            [user_id,x_history,x_inview,label,label_id] = data
            torch.autograd.set_detect_anomaly(True)
            out = model(x_history.to(device), x_inview.to(device))
            ...
            for b_i in range(user_id.shape[0]):
                total_auc += auc_score(label[b_i,:].detach().numpy(),out[b_i,:].cpu().detach().numpy())
